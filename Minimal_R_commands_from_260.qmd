...
`y ~ x | z` can usually be read $y$ is modeled by (or depends on) $x$ for each value of $z$.
...
filter()               # Subset rows 
slice(2:5)             # Select specified rows 
mydata[-c(3,23,36), ]  # Remove specified rows
mydata[, c(2,5)]       # Show only specified columns
...
Here are a few examples. These are not exhaustive but should be seen as representative of the kinds of things done in Applied Statistics. These commands require that the packages **mosaic** and **Lock5Data** be installed (and loaded using the `library()` command) for these examples to work on your computer. For completeness, the following chunk shows how these packages are loaded:
