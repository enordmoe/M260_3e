...
Proportions are based on total sample.  
...
-   Conditional proportions are useful for comparing groups
    - Be sure you know which direction you're conditioning in.
...
-   `position = "fill"` rescales the bars to compare proportions **across groups** for a two-variable relationship.
-   For a **single** categorical variable, `gf_props()` is the more direct choice (no `position` argument needed).
...
| Goal | Command |
|---|---|
| One variable, counts | `gf_bar(~ X)` |
| One variable, proportions | `gf_props(~ X)` |
| Two variables, counts | `gf_bar(~ X, fill = ~ Y)` |
| Two variables, compare proportions | `gf_bar(~ X, fill = ~ Y, position = "fill")` |
| Two variables, side-by-side counts | `gf_bar(~ X, fill = ~ Y, position = "dodge")` |

::: aside
This table appears on your mosaic Quick Reference handout — keep it open during the activity.
:::
