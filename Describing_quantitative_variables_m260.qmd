...
Another plot often of interest is a `boxplot`, which is a graphical depiction of the five number summary (min, $Q_1$, median, $Q_3$, maximum) with outliers, if present, plotted separately. Here is an example using `gf_boxplot()`. Since *Status* is a 0-1 variable, we need to use the `factor()` function to tell R to treat the 0 and 1 as categories.
